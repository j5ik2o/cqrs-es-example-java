...
public class CommandLineRunnerImpl implements CommandLineRunner {
    ...
    private Map<String, AttributeValue> convertAttributeRecordToMap(Map<String, software.amazon.awssdk.services.dynamodb.model.AttributeValue> sdkAttributeValueMap) {
        return sdkAttributeValueMap.map((k, v) -> Map.entry(k, convertAttributeValue(v)));
    }

    private AttributeValue convertAttributeValue(software.amazon.awssdk.services.dynamodb.model.AttributeValue v) {
        if (v.s() != null) {
            ...
            var av = new AttributeValue();
            var m = HashMap.ofAll(v.m()).map((k, v1) -> Map.entry(k, convertAttributeValue(v1))).toJavaMap();
            av.setM(m);
            return av;
        } else {
    ...
    private void streamDriver(DynamoDbClient dynamodbClient, DynamoDbStreamsClient dynamodbStreamsClient, String journalTableName, int maxItemCount) {
        var describeTableResponse = dynamodbClient.describeTable(DescribeTableRequest.builder().tableName(journalTableName).build());
        ...
        for (; ; ) {
            ...
            for (var shard : shards) {
                ...
                var getShardIteratorResponse = dynamodbStreamsClient.getShardIterator(
                ...
                while (shardIterator != null && count < maxItemCount) {
                    var getRecordsResponse = dynamodbStreamsClient.getRecords(
    ...
    private DynamodbEvent convertToEvent(Record record, Map<String, AttributeValue> keys, Map<String, AttributeValue> item, String streamArn) {
        ...
        sr.setStreamViewType(com.amazonaws.services.lambda.runtime.events.models.dynamodb.StreamViewType.valueOf(record.dynamodb().streamViewType().name()));
